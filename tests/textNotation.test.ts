...
describe("core text appearance notation", () => {
  it("compiles opacity, HEX color, and timed disappearance through the shared cue model", () => {
    const compiled = compileTextNotation(
      "/opacity40{ghost} /color#ff8800{ember} /disappear2.5{soon}",
      performance,
    );

    expect(compiled.text).toBe("ghost ember soon");
    expect(compiled.performance.cues).toEqual(expect.arrayContaining([
      expect.objectContaining({ type: "opacity", start: 0, end: 5, value: 40 }),
  ...
  it("accepts short HEX and rejects out-of-range or malformed appearance values", () => {
    expect(validateTextNotation("/color#abc{ok} /opacity0{gone} /disappear0{now}"))
      .toEqual([]);

    const issues = validateTextNotation("/opacity101{x} /color#GG0000{x} /disappear3600.001{x}");
