...
const FEATURE_MUTATIONS = ["node.upsert", "interaction.upsert", "interaction.delete", "entity.upsert", "variable.upsert", "computed.upsert", "item.upsert"] as const;
...
describe("modular architecture composition roots", () => {
  ...
  it("registers installed project-data features with Worker persistence", () => {
    expect(WORKER_FEATURE_PERSISTENCE.map((feature) => feature.id)).toEqual(["narrative", "world", "state", "inventory"]);
  });
  ...
  it("keeps optional workspace routes out of central Author navigation", () => {
