...
function inspectEffects(effects: Effect[], path: string, bodyIds: ReadonlySet<string>, issues: Issue[]) {
  for (const effect of effects) if (effect.type === "set_body_background" && effect.backgroundId && !bodyIds.has(effect.backgroundId)) issues.push({ key: `${path}:${effect.id}:${effect.backgroundId}`, message: "A body-type effect references a body type that has not been saved." });
}
...
function issues(snapshot: ProjectSnapshot) {
  ...
  for (const body of snapshot.bodyTypes) for (const assignment of body.startingEquipment) {
    if (!itemIds.has(assignment.itemId)) found.push({ key: `body.${body.id}.item.${assignment.itemId}`, message: "Starting equipment references an item that has not been saved." });
    if (!body.slots.some((slot) => slot.key === assignment.slotKey)) found.push({ key: `body.${body.id}.slot.${assignment.slotKey}`, message: "Starting equipment references a slot that does not exist on that body type." });
  }
  for (const rule of snapshot.equipmentRules) if (!itemIds.has(rule.itemId)) found.push({ key: `rule.${rule.itemId}`, message: "An equipment rule references an item that has not been saved." });
  for (const interaction of snapshot.interactions) for (const outcome of interaction.outcomes) inspectEffects(outcome.effects, `interaction.${interaction.id}.${outcome.id}`, bodyIds, found);
