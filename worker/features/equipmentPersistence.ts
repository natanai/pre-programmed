...
export const equipmentFeaturePersistence: WorkerFeaturePersistence = {
  ...
  migrations: [
    ...
    {
      id: 27,
      name: "equipment-body-type-effect-vocabulary",
      sql: `
        UPDATE interaction_outcomes
        SET effects_json = replace(
          replace(effects_json, '"type":"set_body_background"', '"type":"set_body_type"'),
          '"backgroundId":',
          '"bodyTypeId":'
        )
        WHERE effects_json LIKE '%set_body_background%';
        ...
        UPDATE project_meta SET schema_version = 27 WHERE id = 1;
      `,
    },
  ],
  ...
  async load(db) {
    ...
    const bodyTypes = bodies.results.map((row): BodyTypeDefinition => ({
      id: row.id,
      name: row.name,
      assetId: row.asset_id,
      slots: parseJson<EquipmentSlotDefinition[]>(row.slots_json, []),
      startingEquipment: parseJson<StartingEquipmentDefinition[]>(row.starting_equipment_json, []),
    }));
    return {
      bodyTypes,
      equipmentRules: rules.results.map((row): EquipmentRuleDefinition => ({
        itemId: row.item_id,
        slotKeys: parseJson(row.slot_keys_json, []),
        storage: row.storage === "slot" ? "slot" : "inventory",
        equipOnGiveSlotKey: row.equip_on_give_slot_key,
      })),
      startingBodyTypeId: settings?.starting_body_type_id && bodyTypes.some((bodyType) => bodyType.id === settings.starting_body_type_id)
        ? settings.starting_body_type_id
        : null,
    };
  },
  ...
  mutationStatements(db, operation) {
    ...
    if (operation.type === "bodyType.delete") return [
      db.prepare("UPDATE equipment_settings SET starting_body_type_id = NULL WHERE starting_body_type_id = ?").bind(operation.id),
      db.prepare("DELETE FROM equipment_body_types WHERE id = ?").bind(operation.id),
    ];

    if (operation.type === "bodyType.starting") {
      return [db.prepare(`INSERT INTO equipment_settings (id, starting_body_type_id) VALUES (1, ?)
        ON CONFLICT(id) DO UPDATE SET starting_body_type_id=excluded.starting_body_type_id`).bind(operation.id)];
    }
    ...
    if (operation.type === "equipmentRule.upsert") {
      ...
      return [db.prepare(`INSERT INTO equipment_item_rules (item_id, slot_keys_json, storage, equip_on_give_slot_key, updated_at)
        VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
        ON CONFLICT(item_id) DO UPDATE SET slot_keys_json=excluded.slot_keys_json, storage=excluded.storage,
          equip_on_give_slot_key=excluded.equip_on_give_slot_key, updated_at=CURRENT_TIMESTAMP`)
  ...
  resetStatements(db) {
    return [
      db.prepare("DELETE FROM equipment_item_rules"),
      db.prepare("DELETE FROM equipment_settings"),
      db.prepare("DELETE FROM equipment_body_types"),
    ];
  },
