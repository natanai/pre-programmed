...
export const statusMutationValidator: WorkerMutationValidator = {
  ...
  validate(operation) {
    ...
    if (operation.type === "statusEntry.upsert") {
      if (!object(operation.entry) || !object(operation.entry.source)) return "Status entry is invalid.";
      ...
      if (!["value", "derived"].includes(String(entry.source.kind))) return "Status entry source kind is invalid.";
      if (typeof entry.source.id !== "string" || !entry.source.id) return "Status entry source is required.";
