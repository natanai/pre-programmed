...
Authored user input uses the same conditional-selection machinery for its response outcomes. An Interaction may preserve ordered first-match priority or deterministically choose among every response whose condition currently matches, using the run's shared world seed plus that Interaction occurrence. Responses and Node Entry Responses both continue through the same ordered Narrative flow: wait for player input, run shared effects, present additional narration/dialogue, and optionally transition to another Node or return to the Node that immediately preceded the current one in traversal history. Author-mode AFTER choices such as STAY HERE, RETURN PREVIOUS, LINK EXISTING, CREATE NEW, and CAPTURE INPUT are presets over that one flow rather than separate runtime systems. A captured submission is exposed through the shared player-input runtime binding, so the canonical Effects editor can store it in State before later steps present text or continue elsewhere. The condition and effect vocabularies are shared across these systems rather than reimplemented by each authoring surface.
...
The **Build portable Windows engine** GitHub Actions workflow is manual-only. To create a versioned Windows package:
...
Recipients extract the whole `Pre-Programmed` folder and run the root `Pre-Programmed.exe` in place. The root stays intentionally small: `Pre-Programmed.exe`, `installation.txt`, `assets/`, and `_engine/`. `_engine/` contains the bundled Electron/Worker runtime and all engine-owned local state. Downloads use the normal Windows Save As dialog instead of an engine-owned exports folder. Nothing is installed system-wide, and Node.js, npm, Git, Cloudflare, and an internet connection are not required to run the packaged engine.
