...
import { configuredAssetStore } from "../../media/ui/assetStore";
...
function CharacterPortraitPreview({ snapshot, assetId, name }: {
  snapshot: Parameters<typeof configuredAssetStore.resolve>[0];
  assetId: string;
  name: string;
}) {
  const portrait = configuredAssetStore.resolve(snapshot, assetId);
  if (!portrait?.available || !portrait.url) {
    return <div className="world-character-portrait-preview is-missing">PORTRAIT MEDIA UNAVAILABLE · {assetId}</div>;
  }
  return <div className="world-character-portrait-preview">
    <img src={portrait.url} alt={`${name || "Character"} portrait`} />
    <small>{portrait.name}</small>
  </div>;
}
          ...
          children: [
            ...
            ...(draft.type === "character" ? [{
              ...
              content: <div className="world-character-portrait-field">
                ...
                />
                {draft.portraitAssetId ? <CharacterPortraitPreview snapshot={context.snapshot} assetId={draft.portraitAssetId} name={draft.name} /> : null}
              </div>,
            }] : []),
