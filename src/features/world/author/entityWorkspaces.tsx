...
function entityRoute(type: EntityDefinition["type"], id?: string, resourceTask = false) {
  return {
    type: "feature" as const,
    feature: "world",
    workspace: "entity",
    data: { entityType: type, ...(id ? { resourceId: id } : {}), ...(resourceTask ? { resourceTask: type } : {}) },
  };
}
...
export const worldLibraryWorkspace = defineAuthorWorkspace({
  ...
  createDraft: () => ({}),
  buildSpec: ({ context }) => ({
    id: "world-library", title: "People + places", context: `${context.snapshot.entities.length} world entries`,
    blocks: [{ type: "custom", id: "world-entities", role: "results", content: <div className="author-ui-resource-list">
      <button type="button" onClick={() => context.pushTask(entityRoute("character"))}>[+ CHARACTER]</button>
      <button type="button" onClick={() => context.pushTask(entityRoute("location"))}>[+ LOCATION]</button>
      {context.snapshot.entities.map((entity) => <button type="button" key={entity.id} onClick={() => context.pushTask(entityRoute(entity.type, entity.id))}>
        {entity.name || entity.key} <small>{entity.type}</small>
      </button>)}
    </div> }],
  }),
});
...
export const worldEntityWorkspace = defineAuthorWorkspace<EntityDefinition>({
  ...
  async save({ route, context, draft }) {
    const key = resolveAuthorKey({ override: draft.key, source: draft.name, existingKeys: context.snapshot.entities.filter((entity) => entity.id !== draft.id).map((entity) => entity.key), fallback: draft.type });
