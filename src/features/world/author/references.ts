...
export const worldProjectReferences: ProjectReferenceContribution = (snapshot, context) => snapshot.entities.flatMap((entity) =>
  hookTargets(context, entity.hooks ?? []).map((target) => ({
    ...target,
