...
export function DefinitionsPanel({ snapshot, onSave, onClose }: {
  ...
  return <section className="author-panel definitions-panel" onPointerDown={(event) => event.stopPropagation()}>
    ...
    {mode === "variables" ? <>
      <div className="definition-list">{snapshot.variables.map((item) => <button type="button" key={item.id} onClick={() => setVariable(structuredClone(item))}><span>{item.label}</span><span>{item.key} : {item.valueType}</span></button>)}</div>
      <button type="button" onClick={() => setVariable({ id: crypto.randomUUID(), key: "", label: "", valueType: "number", initialValue: 0, showInStatus: false })}>[+ VARIABLE]</button>
      ...
      <div className="definition-list">{snapshot.computedValues.map((item) => <button type="button" key={item.id} onClick={() => setComputed(structuredClone(item))}><span>{item.label}</span><span>{item.key} : {item.source}</span></button>)}</div>
      <button type="button" onClick={() => setComputed({ id: crypto.randomUUID(), key: "", label: "", source: "elapsed_seconds", format: "integer", showInStatus: false })}>[+ COMPUTED VALUE]</button>
