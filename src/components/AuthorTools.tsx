...
export function WorkspacePanel({ token, snapshot, playState, initialView = "locations", onSave, onSnapshot, onRestore, onClose: _onClose }: {
  token: string;
  snapshot: ProjectSnapshot;
  playState: PlayState;
  ...
  const createBookmark = async () => {
    ...
    const bookmark: AuthorBookmark = { id: crypto.randomUUID(), nodeId: savedState.currentNodeId, traversal: savedState.traversal, playState: savedState, note: label, createdAt: new Date(savedAt).toISOString() };
