...
export function NodeEditor({ node, snapshot, onSave, onCancel }: {
  ...
  return <section className="author-panel node-editor" onPointerDown={(event) => event.stopPropagation()}>
    <header><span>NODE #{draft.nodeNumber}</span><button type="button" onClick={onCancel}>[X]</button></header>
    <label>OUTPUT <textarea ref={textarea} rows={5} value={draft.text} onChange={(event) => setDraft({ ...draft, text: event.target.value })} autoFocus /></label>
    ...
    <div className="node-meta-grid">
      ...
      <label>ENDING <input type="checkbox" checked={draft.ending} onChange={(event) => setDraft({ ...draft, ending: event.target.checked })} /></label>
    ...
    <div className="author-actions"><button type="button" disabled={saving} onClick={() => void save()}>[{saving ? "SAVING..." : "SAVE"}]</button><button type="button" onClick={onCancel}>[CANCEL]</button></div>
  </section>;
}
