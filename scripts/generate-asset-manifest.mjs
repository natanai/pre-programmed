...
async function readIdentity(path) {
  ...
  if (value.authoringMode !== undefined && !["file", "grid32"].includes(value.authoringMode)) throw new Error(`Asset sidecar ${sidecarPath} has an invalid authoringMode.`);
  return value;
}
...
for (const path of files.sort()) {
  ...
  assets.push({
    ...
    authoringMode: identity.authoringMode ?? "file",
  });
}

const output = `// Generated by scripts/generate-asset-manifest.mjs. Do not hand-edit.\n` +
  `export type AssetManifestEntry = { id: string; name: string; path: string; runtimePath: string; type: "image" | "audio"; mimeType: string; byteLength: number; hash: string; dimensions: { width: number; height: number } | null; defaultPresentation: "inline" | "overlay"; authoringMode: "file" | "grid32" };\n` +
